...
// ################ Check Offshore Count Total ################ 
// var GOM_boundary_union = GOM_boundary.union()
// var GOM_boundary_geometry= ee.Geometry(GOM_boundary_union.geometry());
var GOM_boundary_geometry = GOM_boundary;
var in_GOM = function(fc){
  fc = ee.FeatureCollection(fc);
  var output = fc.filter(ee.Filter.bounds(GOM_boundary_geometry));
  return ee.FeatureCollection(output);
}; var GOM_well_FB = Well_List.map(in_GOM);
...
var GOM_well_FB_count = GOM_well_FB.map(well_count);
print('count of shale wellt/total well, by well type'); 
print(Well_List_names.zip(shale_well_FB_count).zip(GOM_well_FB_count).zip(count));
