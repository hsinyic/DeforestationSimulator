...
var shale_wells = ee.FeatureCollection(Well.map(function(feature){feature = ee.Feature(feature);
  return(feature.set("shale_play", 1));
}))
// .filterBounds(buffer_shaleplay)
.filter(ee.Filter.bounds(shaleplay));

var set_shale = function(feature){
  feature = ee.Feature(feature);
  return(feature.set("shale_play", 2));
}

shale_wells = ee.FeatureCollection(shale_wells.map(set_shale));
var shale_label = shale_wells.merge(Well);
Export.table.toDrive(shale_label, "shale_label2");
...
var offshore_wells = ee.FeatureCollection(Well)
// .filterBounds(buffer_shaleplay)
.filter(ee.Filter.bounds(GOM_boundary))

var set_offshore = function(feature){
  feature = ee.Feature(feature);
  return(feature.set("offshore", 2));
}
offshore_wells = ee.FeatureCollection(offshore_wells.map(set_offshore));

var offshore_label =  offshore_wells.merge(shale_label);

Export.table.toDrive(offshore_label, "offshore_label2");


// Export.table.toDrive(invertedJoined, "compilation");
