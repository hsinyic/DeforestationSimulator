...
/*var select = function( after,before, direction, type){
  after = ee.String(after).cat('-01-01');before= ee.String(before).cat('-01-01');
  var first = ee.Filter.rangeContains('First Prod Date', ee.Date(after).millis(), ee.Date(before).millis() )
  var completion = ee.Filter.rangeContains('Completion Date', ee.Date(after).millis(), ee.Date(before).millis() )
  var spud = ee.Filter.rangeContains('Spud Date', ee.Date(after).millis(), ee.Date(before).millis() )
  var time_filter = ee.Filter.or(first, completion, spud);
  direction = ee.String(direction);
  type= ee.List(type);  // print(before, after, direction, type, ee.Date(after).millis(), ee.Date(before).millis())
  var selected_fc = ee.FeatureCollection(Well
  .filter(time_filter)
...
var firstN = ee.Filter.eq('First Prod Date', null )
var completionN = ee.Filter.eq('Completion Date', null)
var spudN = ee.Filter.eq('Spud Date', null )
var timeN_filter = ee.Filter.and(firstN, completionN, spudN);
print(Well.filter(timeN_filter).aggregate_sum("Well Count"));
*/
...
// Map.addLayer(ee.FeatureCollection(shale_well_FB.get(12)), {'color':azure}, s_list_names.get(12).getInfo(), 0, 1); 
// Map.addLayer(ee.FeatureCollection(shale_well_FB.get(13)), {'color':bl}, s_list_names.get(13).getInfo(), 0, 1); 
